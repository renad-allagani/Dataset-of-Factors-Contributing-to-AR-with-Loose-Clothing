...
expName = {'FlatSurface-perpendicular','InclineSurface-perpendicular','FlatSurface-parallel', 'InclineSurface-parallel'};

freq = {'/150/low','/300/high'};

for expIndex = 1:2:length(expName) %two experiments
    ...
    for freqIndex = 1:length(freq)
        ...
        for trial_number = 1 : repeats
            low_data(startRow:endRow,startColumn:endColumn,trial_number) = readmatrix(['DataCollection_RobotArm/', expName{expIndex},freq{freqIndex},num2str(trial_number),'.csv'], 'Range', [startRow startColumn endRow endColumn]);
            high_data(startRow:endRow,startColumn:endColumn,trial_number) = readmatrix(['DataCollection_RobotArm/', expName{expIndex+1},freq{freqIndex},num2str(trial_number),'.csv'], 'Range', [startRow startColumn endRow endColumn]);
        end
...
%         fileName = sprintf('workspace_%s_freq%s.mat',orientation,fequency);
%         save(['Workspaces_RobotArm_Plane/',fileName]);
